...
CurvePlot::CurvePlot(QWidget *parent) : QCustomPlot(parent){
...
//	this->axisRect()->setMargins(QMargins(2,2,2,2));

	this->panel = new SliderPanel(this);
	...
	connect(this->panel->checkBoxProcessed, &QCheckBox::stateChanged, this, &CurvePlot::slot_displayProcessed);
	connect(this->panel->checkBoxRaw, &QCheckBox::stateChanged, this, &CurvePlot::slot_displayRaw);
	connect(this->panel->checkBoxAutoscale, &QCheckBox::stateChanged, this, &CurvePlot::slot_activateAutoscaling);
	connect(this->panel->spinBoxLine, QOverload<int>::of(&QSpinBox::valueChanged), this, &CurvePlot::slot_setLine);
}

CurvePlot::~CurvePlot(){
}

void CurvePlot::setRawAxisColor(QColor color) {
...
void CurvePlot::setProcessedAxisColor(QColor color) {
...
void CurvePlot::setRawPlotVisible(bool visible) {
...
void CurvePlot::setProcessedPlotVisible(bool visible) {
	int transparency = visible ? 255 : 0;
...
void CurvePlot::contextMenuEvent(QContextMenuEvent *event){
	QMenu menu(this);
	...
	connect(&bitshiftRawValuesAction, &QAction::toggled, this, &CurvePlot::slot_enableBitshift);
	...
	connect(&savePlotAction, &QAction::triggered, this, &CurvePlot::slot_saveToDisk);
	menu.addAction(&savePlotAction);
	menu.exec(event->globalPos());
}

void CurvePlot::slot_plotRawData(void* buffer, unsigned bitDepth, unsigned int samplesPerLine, unsigned int linesPerFrame, unsigned int framesPerBuffer, unsigned int buffersPerVolume, unsigned int currentBufferNr){
...
void CurvePlot::slot_plotProcessedData(void* buffer, unsigned bitDepth, unsigned int samplesPerLine, unsigned int linesPerFrame, unsigned int framesPerBuffer, unsigned int buffersPerVolume, unsigned int currentBufferNr) {
...
void CurvePlot::slot_changeLinesPerBuffer(int linesPerBuffer){
	this->linesPerBuffer = linesPerBuffer;
	this->panel->setMaxLineNr(linesPerBuffer);
}

void CurvePlot::slot_setLine(int lineNr){
	this->line = lineNr;

}

void CurvePlot::slot_displayRaw(bool display){
	this->displayRaw = display;
	this->setRawPlotVisible(display);
	this->replot();
}

void CurvePlot::slot_displayProcessed(bool display){
	this->displayProcessed = display;
	this->setProcessedPlotVisible(display);
	this->replot();
}

void CurvePlot::slot_activateAutoscaling(bool activate) {
	this->autoscaling = activate;
	this->replot();
}

void CurvePlot::slot_saveToDisk() {
...
void CurvePlot::slot_enableRawGrabbing(bool enable) {
	this->rawGrabbingAllowed = enable;
}

void CurvePlot::slot_enableProcessedGrabbing(bool enable) {
	this->processedGrabbingAllowed = enable;
}

void CurvePlot::slot_enableBitshift(bool enable) {
...
void SliderPanel::setMaxLineNr(unsigned int maxLineNr){
